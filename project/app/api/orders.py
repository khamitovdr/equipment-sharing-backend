...
#         chat_credentials = ChatCredentialsSchema(
...
#     order.chat_credentials = chat_credentials
#     return order


@router.get("/{order_id}/chat-credentials", response_model=ChatCredentialsSchema)
async def get_order_chat_credentials_(order_id: int, current_user: User = Depends(get_current_active_user)):
    """Get order by id"""
    order = await get_order_by_id(order_id)
    user_organization = await current_user.organization
    if order is None:
